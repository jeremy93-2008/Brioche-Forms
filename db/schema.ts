...
export const tagsFormsTable = sqliteTable("tags_forms", {
    id: text("id").primaryKey(),
    tag_id: text("tag_id").notNull().references(() => tagsTable.id),
    form_id: text("form_id").notNull().references(() => formsTable.id),
})
...
export const choicesTable = sqliteTable("choices", {
    id: text("id").primaryKey(),
    question_id: text("question_id").notNull().references(() => questionsTable.id),
    content: text("content").notNull(),
    order: text("order").notNull(),
});
...
export const multipleChoicesTable = sqliteTable("multiple_choices", {
    id: text("id").primaryKey(),
    answer_id: text("answer_id").notNull().references(() => answersTable.id),
    choice_id: text("choice_id").notNull().references(() => choicesTable.id),
});

export const answersTable = sqliteTable("answers", {
    id: text("id").primaryKey(),
    response_id: text("response_id").notNull().references(() => responsesTable.id),
    question_id: text("question_id").notNull().references(() => questionsTable.id),
